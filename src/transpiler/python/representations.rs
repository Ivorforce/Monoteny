...
pub fn find_for_function(forms: &mut HashMap<Rc<FunctionHead>, FunctionForm>, global_namespace: &mut namespaces::Level, implementation: &FunctionImplementation) {
    if implementation.parameter_locals.is_empty() {
        ...
        if !has_blocks {
            global_namespace.insert_name(implementation.head.function_id, implementation.head.declared_representation.name.as_str());
