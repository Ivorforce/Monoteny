...
pub fn add_nominal(runtime: &mut Runtime, module: &mut Module, scope: Option<&mut scopes::Scope>, trait_: &Rc<Nominal>) -> RResult<()> {
    ...
    runtime.source.nominal_references.insert(
        Rc::clone(&getter),
        Rc::clone(trait_),
    );
    runtime.source.fn_logic.insert(
        Rc::clone(&getter),
        FunctionLogic::Descriptor(FunctionLogicDescriptor::NominalProvider(Rc::clone(trait_))),
    );
...
pub fn add_function(runtime: &mut Runtime, module: &mut Module, scope: Option<&mut scopes::Scope>, function: &Rc<FunctionHead>) -> RResult<()> {
    // TODO Once functions are actually objects, we can call add_nominal from here.
    // The type of a specific function value is concrete (a struct) that conforms to `Function`.
    let function_trait = Rc::new(Nominal::new_struct(&function.declared_representation.name));
    ...
    runtime.source.function_nominals.insert(Rc::clone(&function_trait), Rc::clone(&function));
